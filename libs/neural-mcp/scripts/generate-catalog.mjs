...
const editorRoot = join(workspaceRoot, 'libs/neural-editor');
const editorPackage = await readJson(join(editorRoot, 'package.json'));
...
publicDirectoryEntries.set('../neural-editor', '@neural-ng/editor');
const sourceFiles = [
  ...(await listFiles(coreRoot)),
  ...(await listFiles(editorRoot)),
];
const trackedFiles = new Set([
  join(workspaceRoot, 'tsconfig.base.json'),
  join(coreRoot, 'package.json'),
  join(editorRoot, 'package.json'),
]);
const components = [];
...
const packageCatalog = {
  ...
  companionPackages: [
    {
      packageName: editorPackage.name,
      version: editorPackage.version,
      runtimeEntryPoints: ['@neural-ng/editor'],
      documentedExports: Object.keys(editorPackage.exports ?? {}).sort(
        compareText,
      ),
    },
  ],
